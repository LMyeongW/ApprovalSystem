...
<script>
	...
	function reviewCompleteList(num){
		...
		$('#reviewer').val(namegrade);
		var reviewer = $('#reviewer').val();
		
		
		$.ajax({
			
			url : '/mtsystem/reviewCompleteList.do',
			type : 'get',
			dateType :'json',
			data : {
				"USER_NUM" : usernum,
			...
			success : function(result){
				...
				var data = result.data.reviewCompleteList;
				var count = result.data.reviewCompleteListCount;
				...
				for(var i = 0; i < data.length; i++){
					...
					html += '<td><a href="/mtsystem/private/reviewRegister?APPROVAL_PK='+data[i].approval_PK +'">확인</a></td>';
					html += '</tr>';
				}
				...
				if(paging.prev){
					...
					page += '<a href="javascript:reviewCompleteList(' + startPageNum +')" id="prev"><span></span><span></span></a>';
				...
				for(var num = paging.startPageNum; num <= paging.endPageNum; num++ ){
					if(select == num){
						...
						page += '<span class="point"><a href="javascript:reviewCompleteList(' + num + ')">' + num + '</a></span>';
						page +=	'</span>';
					}
					
					if(select != num){
						page += '<span class="numPadding">';
						page += '<span class="noPoint"><a href="javascript:reviewCompleteList(' + num + ')">' + num + '</a></span>';
						page +=	'</span>';
					}
				}
				...
				if(paging.next){
					...
					page +=	'<a href="javascript:reviewCompleteList(' + endPageNum +')" id="next"><span></span><span></span></a>';
