            ...
            <div class="right_content">
                ...
                <div class="con_right">
                    ...
                    <div class="reviewRegister">
                            ...
                            <table>
                                <tbody>
                                    <tr>
                                        <th>검토 상태</th>
                                        <td>
                                            <input type="radio" name="REVIEW_STATUS" id="re1" value ="검토 완료" <c:if test="${data.approvalvo.REVIEW_STATUS eq '검토 완료'}">checked="checked"</c:if>/>
                                            <label for="re1">검토완료</label>

                                            <input type="radio" name="REVIEW_STATUS" id="re2" value="반려" <c:if test="${data.approvalvo.REVIEW_STATUS eq '반려'}">checked="checked"</c:if>/>
                                            <label for="re2">반려</label>

                                            <input type="radio" name="REVIEW_STATUS" id="re3" value="검토 대기중" <c:if test="${data.approvalvo.REVIEW_STATUS eq '검토 대기중'}">checked="checked"</c:if>/>
                                    ...
                                    <tr>
                                        <th>검토 내용</th>
                                        <td>
                                            <textarea id="review_content" name="REVIEW_CONTENT">${data.approvalvo.REVIEW_CONTENT}</textarea>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                        ...
                        <c:if test="${member.USER_NUM != data.approvalvo.REVIEW_KEY}" >
            				<script>
...
<script>
    $(document).ready(function () {
		...
		$('.review_ok').click(function(){
			...
			if(con){
				var data = $('form[name=formReview]').serialize();
				
				var reviewer =  $('input[name=REVIEWER]').val();
				var pk = $('input[name=APPROVAL_PK]').val();
				var contnet = $('textarea[name=REVIEW_CONTENT]').val();
				var status = $('input[name=REVIEW_STATUS]:checked').val();
				
				
				$.ajax({
					
					url : '/mtsystem/reviewRegister.do',
					type : 'post',
					dataType :'json',
					data : {
						"APPROVAL_PK" : pk,
						"REVIEW_STATUS" : status,
						"REVIEW_CONTENT" : contnet,
						"REVIEWER" : reviewer
					...
					success : function(result){
						...
						let data = {
								...
								"REVIEW_CONTENT" : result.data.review_CONTENT
							};
	
						
						$.ajax({
							url : '/mtsystem/reviewDate.do',
							type : 'post',
							dataType :'json',
							data : data,
							success : function(result){
